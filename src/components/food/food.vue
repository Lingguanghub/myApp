        ...
        <div class="rating">
          ...
          <div class="rating-wrapper">
            <ul v-show="food.ratings && food.ratings.length">
              <li v-for="(rating,index) in food.ratings" :key="index" class="rating_item border-1px"
                v-show="needShow(rating.rateType,rating.text)">
                <div class="user">
                  <span class="name">{{ rating.username }}</span>
                  <img :src="rating.avatar" class="avatar" width="12" height="12"/>
                </div>
                <!--通过 | formatDate给时间戳过滤,从而达到我们想要的时间格式-->
            ...
            <!--========没有评价的情况-->
            <div class="not-rating" v-show="!food.ratings || !food.ratings.length">暂无评价</div>
          </div>
        </div>
      </div>
    </div>
